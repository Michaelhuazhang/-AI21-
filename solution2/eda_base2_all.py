...
res[['instance_id', 'predicted_score']].to_csv("./final_submit/edabase2all_%f_%s.csv" % (loss/5, now), index=False)
...
train_prob.to_csv("./stacking/edabase2alltrain_cv%f.csv"%(loss/5),index=False)

test_prob=pd.DataFrame(res['predicted_score'])
test_prob.columns=['click']
test_prob.to_csv("./stacking/edabase2alltest_cv%f.csv"%(loss/5),index=False)
