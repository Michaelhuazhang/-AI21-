...
cntTf = cntVector.fit_transform(user_tags)
lda = LatentDirichletAllocation(n_topics=6,
                                learning_offset=50.,
                                random_state=666)
docres = lda.fit_transform(cntTf)
train = pd.concat([train,pd.DataFrame(docres)],axis=1)
...
res[['instance_id', 'predicted_score']].to_csv("./final_submit/edabase3all_%f_%s.csv" % (loss/5, now), index=False)
...
train_prob.to_csv("./stacking/edabase3alltrain_cv%f.csv"%(loss/5),index=False)

test_prob=pd.DataFrame(res['predicted_score'])
test_prob.columns=['click']
test_prob.to_csv("./stacking/edabase3alltest_cv%f.csv"%(loss/5),index=False)
