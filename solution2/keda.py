...
res[['instance_id', 'predicted_score']].to_csv("./final_submit/keda_%f_%s.csv"% (loss/5, now), index=False)

#train_result_df["predict"] = out_of_fold_predictions[:, 0]
#train_result_df[['instance_id', 'predict']].to_csv("train_keda_%s.csv" % now, index=False)

#保存train_prob
train_prob=pd.DataFrame(out_of_fold_predictions[:, 0])
train_prob.columns=['click']
train_prob.to_csv("./stacking/kedatrain_cv%f.csv"%(loss/5),index=False)

test_prob=pd.DataFrame(res['predicted_score'])
test_prob.columns=['click']
test_prob.to_csv("./stacking/kedatest_cv%f.csv"%(loss/5),index=False)
