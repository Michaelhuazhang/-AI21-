...
if os.path.exists('base_alltrain_csr.npz') and True:
    print('load_csr---------')
    base_train_csr = sparse.load_npz('base_alltrain_csr.npz').tocsr().astype('bool')
    base_predict_csr = sparse.load_npz('base_allpredict_csr.npz').tocsr().astype('bool')
else:
    ...
    sparse.save_npz('base_alltrain_csr.npz', base_train_csr)
    sparse.save_npz('base_allpredict_csr.npz', base_predict_csr)
...
predict_result[['instance_id', 'predicted_score']].to_csv("./final_submit/kdxfall_%f_%s.csv" % (np.mean(best_score), now), index=False)


train_result_df["click"] = out_of_fold_predictions[:, 0]
train_result_df[[ 'click']].to_csv("./stacking/kdxf_alltrain_%f_%s.csv" % (np.mean(best_score), now), index=False)
test_prob=pd.DataFrame(predict_result['predicted_score'])
test_prob.columns=['click']
test_prob.to_csv("./stacking/kdxfall_cv%f.csv"%(np.mean(best_score)),index=False)
