...
sub.to_csv("./final_submit/feature_test_score_all_%f.csv"%logloss,index=False)
...
train_prob.to_csv("./stacking/lgb_alltrain_prob_cv%f.csv"%(logloss),index=False)

test_prob=pd.DataFrame(oof_test)
test_prob.columns=['click']
test_prob.to_csv("./stacking/lgb_alltest_prob_cv%f.csv"%(logloss),index=False)
