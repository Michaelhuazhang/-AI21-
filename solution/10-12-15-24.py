...
train_result_df.to_csv("./stacking/team1_train_%f.csv" % (np.mean(best_score)), index=False)
test_prob=pd.DataFrame(predict_result['predicted_score'])
test_prob.columns=['click']
test_prob.to_csv("./stacking/team1_cv%f.csv"%(np.mean(best_score)),index=False)
